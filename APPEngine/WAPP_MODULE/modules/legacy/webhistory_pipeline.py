...
from ...parsers.legacy.WebHistoryParser import WebHistoryParser
...
class WebHistoryPipeline(BaseArtefactPipeline):
    ...
    def process(self, file_path: Path):
        self.logger.info(f"[PIPELINE][WEBHISTORY] Processing {file_path.name}", header="START", indentation=1)
        try:
            ...
            self.logger.info(f"[PIPELINE][WEBHISTORY] Success", header="FINISHED", indentation=1)
        except Exception as e:
            self.logger.error(f"[PIPELINE][WEBHISTORY] Error on {file_path.name}: {e}", header="ERROR", indentation=1)
