...
@register_pipeline(name="ogre_process")
class OgreProcessPipeline(BaseArtefactPipeline):
    """
    Parses DFIR-Ogre process output files.
    """
    recommended = True
    ...
    def process(self, file_path: Path):
        self.logger.info(f"[PIPELINE][OGRE_PROCESS] Processing {file_path.name}", header="START", indentation=1)
        try:
            
            self.context.siem_ingestion_files.append(str(file_path))
            self.context.wazuh_importer_file_config["files"].append({"path": str(file_path), "type": "process"})
            ...
            self.logger.info(f"[PIPELINE][OGRE_PROCESS] Success for {file_path.name}", header="FINISHED", indentation=1)
        except Exception as e:
            self.logger.error(f"[PIPELINE][OGRE_PROCESS] Error on {file_path.name}: {e}", header="ERROR", indentation=1)
