...
@register_pipeline(name="ogre_mft")
class OgreMftPipeline(BaseArtefactPipeline):
    """
    Parses DFIR-Ogre MFT/NTFSInfo output files.
    """
    recommended = True
    importance = "Highly recommended"
    speed = "Fast"
    DEFAULT_PATTERNS = {"ogre_mft": [r".*\.ntfsinfo\.jsonl$", r".*\.i30info\.jsonl$"]}
    ...
    def process(self, file_path: Path):
        self.logger.info(f"[PIPELINE][OGRE_MFT] Processing {file_path.name}", header="START", indentation=1)
        try:
            
            self.context.siem_ingestion_files.append(str(file_path))
            self.context.wazuh_importer_file_config["files"].append({"path": str(file_path), "type": "mft"})
            ...
            self.logger.info(f"[PIPELINE][OGRE_MFT] Success for {file_path.name}", header="FINISHED", indentation=1)
        except Exception as e:
            self.logger.error(f"[PIPELINE][OGRE_MFT] Error on {file_path.name}: {e}", header="ERROR", indentation=1)
